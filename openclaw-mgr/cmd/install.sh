...
printf '\n  ② 컨테이너 안 CLI (네트워크 모드 무관, 본체 OpenClaw 사용):\n'
printf '       %scd ~/DEV/openclaw%s\n' "$C_BOLD" "$C_RESET"
printf '       %sdocker compose run --rm openclaw-cli onboard%s        # 첫 설정 (한 번만)\n' "$C_BOLD" "$C_RESET"
printf '       %sdocker compose run --rm openclaw-cli tui%s            # 터미널 UI 채팅\n' "$C_BOLD" "$C_RESET"
printf '       ※ openclaw-cli 는 entrypoint 가 즉시 종료형이라 %sexec 가 아니라 run --rm%s 으로 호출합니다\n' "$C_BOLD" "$C_RESET"
printf '\n  ③ 터미널 REPL 채팅 (네트워크 모드 무관, 호스트 Ollama 직접, 설정 불필요):\n'
