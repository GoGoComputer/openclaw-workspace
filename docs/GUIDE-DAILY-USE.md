...
| Discord 봇 응답: 같은 generic 에러, 로그는 **`agent/embedded Profile ollama:default timed out`** / **`llm-idle-timeout: ollama/<model> produced no reply before the idle watchdog`** | **모델이 너무 무거워 첫 응답 전에 OpenClaw idle watchdog 트리거**. `gemma4:26b` (17GB) 같은 거 24GB RAM 에서 첫 로드 55+초 걸려 timeout. 가벼운 모델로 바꾸기 — `llama3.1:8b-instruct-q4_K_M` (4.9GB) 추천. `python3 -c 'import json; p="/Users/mo/.openclaw/openclaw.json"; cfg=json.load(open(p)); m=cfg["models"]["providers"]["ollama"]["models"]; pref="llama3.1:8b-instruct-q4_K_M"; cfg["models"]["providers"]["ollama"]["models"]=[x for x in m if x["id"]==pref]+[x for x in m if x["id"]!=pref]; json.dump(cfg, open(p,"w"), indent=2)'` 후 `./openclaw stop && start`. config 모델 배열의 **첫 항목**이 default 가 됨. 사전 워밍업: `curl -X POST http://127.0.0.1:11434/api/generate -d '{"model":"llama3.1:8b-instruct-q4_K_M","prompt":"hi","stream":false}'`. |
