...
| Discord 봇 응답: 위와 같은 메시지인데 로그는 **`Failed to inspect sandbox image: permission denied while trying to connect to the docker API`** | docker.sock 마운트는 있는데 **컨테이너 안 node 사용자가 그 sock 에 접근 권한이 없음**. macOS Docker Desktop 에선 마운트된 sock 가 `root:root` 로 보이는데 (호스트 GID `daemon`(1) 과 다름), `group_add` 가 `1` 만 추가하던 게 회귀. v0.2.19+ `step_sandbox` 가 `0`(root) 도 함께 추가. 즉시 fix: sandbox compose 의 `group_add` 에 `"0"` 추가 후 재시작. 검증: `docker exec openclaw-openclaw-gateway-1 docker info --format '{{.ServerVersion}}'` 가 버전을 반환. |
| Discord 봇 응답: 같은 generic 에러, 로그는 **`mounts denied: The path /home/node/.openclaw/sandboxes/... is not shared from the host and is not known to Docker`** | OpenClaw 본체가 sandbox sub-container 띄울 때 mount source 로 **컨테이너 내부 경로**를 그대로 daemon 에 넘김 → daemon 이 호스트의 그 경로를 찾으려다 실패. OpenClaw upstream 한계 (호스트 경로 변환 env 없음). **즉시 fix (v0.2.20)**: `agents.defaults.sandbox.mode = "off"` 로 sandbox sub-container 자체를 비활성화 — 자세한 트레이드오프와 patch 명령은 [GUIDE-DISCORD-BOT: 케이스 C](GUIDE-DISCORD-BOT.md#봇이-응답으로-something-went-wrong-while-processing-your-request-please-try-again-or-use-new-to-start-a-fresh-session). |
| Discord 봇 응답: 같은 generic 에러, 로그는 **`agent/embedded Profile ollama:default timed out`** / **`llm-idle-timeout: ollama/<model> produced no reply before the idle watchdog`** | **모델이 너무 무거워 첫 응답 전에 OpenClaw idle watchdog (2분) 트리거**. `gemma4:26b` (17GB) 는 55+초, `llama3.1:8b-instruct-q4_K_M` (4.6GB) 도 system prompt + workspace 7개 .md 컨텍스트면 종종 timeout. **v0.2.23 권장 (24GB RAM): `gemma4:latest` (8.9GB)** — cold 7.2초 / warm 0.27초, vision+audio+thinking+tools. 16GB 이하면 `qwen2.5:7b-instruct` (4.7GB), 8GB 면 `qwen2.5:3b-instruct` (1.8GB). 영구 lock: `.env` 에 `OPENCLAW_FORCE_DEFAULT_MODEL="gemma4:latest"`. 자세히 → [GUIDE-DISCORD-BOT: 케이스 D](GUIDE-DISCORD-BOT.md#케이스-d--모델이-너무-무거워-첫-응답-전에-watchdog-트리거-v021--v022-권장-모델-변경) |
| Discord 봇 응답이 평문 JSON 으로 나옴 (`{"name": "read", "parameters": {...}}` 같은 거) | **케이스 D 의 부산물** — 별개 버그 아님. 모델이 도구 호출 객체를 만들다가 watchdog 가 끊어서 incomplete JSON 이 그대로 사용자한테 전달. Fix 는 케이스 D 와 동일 (가벼운 모델로 default 변경). 한때 case F 로 의심됐지만 모델 빠르면 자동 해결. |
| Discord 봇이 빠르게 응답하긴 하는데 **직접 명령 ("지워줘", "채워줘", "기억해줘") 을 안 따라감** — 대신 "Would you like me to…" 식으로 대화로 빠짐 | **모델 instruction-following 한계** (case G — v0.2.23 신규). 흔히 3B 이하 모델 (`qwen2.5:3b-instruct`, `llama3.2:3b` 등) 이 다단계 reasoning + 도구 호출 결정 약함. Fix: 더 큰 모델로 — 24GB RAM 이면 `gemma4:latest` 추천 (vision+audio+thinking+tools, 8.9GB). 영구화는 `.env` 에 `OPENCLAW_FORCE_DEFAULT_MODEL="gemma4:latest"` + `./openclaw setup --skip-confirm` 한 번. |
| Discord 봇 응답: 같은 generic 에러, 로그는 **`channel exited: Cannot find module '.../plugin-runtime-deps/openclaw-unknown-<hash>/...text-runtime.js'`** 또는 **`ENOENT: ... account-core.js`** | OpenClaw 가 컨테이너 안에서 자기 버전 못 읽고 **`openclaw-unknown-<hash>` 디렉토리를 새로 만들면서 plugin-sdk 를 부분적으로만 채움** (text-runtime.js / account-core.js 누락). 보통 host-paths 실험이나 env 변경 후 발생. **Fix**: `trash ~/.openclaw/plugin-runtime-deps` (또는 `rm -rf`) 후 `./openclaw stop && start` — OpenClaw 가 다음 부팅 시 26개 dep 새로 정상 설치. 로그에 `discord installed bundled runtime deps in <N>ms: @buape/carbon, @discordjs/voice, ...` 보이면 OK. |
