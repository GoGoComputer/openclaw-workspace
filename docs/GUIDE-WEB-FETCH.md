...
이 문서는 **OpenClaw 컨테이너를 인터넷에 잠깐만 노출시키는 안전한 패턴 + `surf` 1회용 샌드박스 사용법**을 다룹니다. 한국어/영어 거울 구조:

| 부 | 내용 |
|---|---|
| 🇰🇷 [한국어 — 핵심 요약 → 8절](#-한국어) | 1️⃣ 권장 사이클 (online↔isolated) → 2️⃣ 실전 프롬프트 → 3-5️⃣ 출처·API·안전 → 6️⃣ 자동화 → 7️⃣ 트러블슈팅 → **8️⃣ `surf` 1회용 샌드박스 (가장 쉬운 길)** |
| 🇬🇧 [English](#-english) | Same content, mirror |

> 🎯 **첫 방문 권장**: 위의 5분 빠른 시작 (TL;DR) 만 따라하면 끝. 더 자세히 알고 싶으면 §1 권장 사이클 → §5 안전 패턴 순. **실시간 데이터를 매일 자동으로** 받고 싶으면 §6 자동화 또는 §8 `surf`.
...
| [🔄 일상 사용](GUIDE-DAILY-USE.md) | 매일 켜고 끄는 사이클 — 시나리오별 절차·종료 방법별 비교·유지보수 |
