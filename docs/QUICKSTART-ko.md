...
이 문서는 **순서대로 따라가는 9단계 레시피** 입니다. 위에서 아래로 한 줄씩 복사·붙여넣기만 하면 됩니다.

| 단계 | 내용 |
|---|---|
| 1–4 | 터미널 열기 → 코드 받기 → 폴더 이동 → `.env` 자동 생성 |
| 5 | `./openclaw doctor` — 뭐가 빠졌는지 확인 |
| 6 | `./openclaw install` — 빠진 부분 자동 보충 |
| 7 | 잘 되는지 확인 |
| 8–9 | 매일 자동 업데이트 + 안전 백업 |

> 🎯 **권장**: 위에서 아래로 한 번에 (~10분). 막히는 곳 있으면 [문제가 생기면](#문제가-생기면) 또는 [TROUBLESHOOTING.md](TROUBLESHOOTING.md).
