...
이 문서는 **Pinterest → 미리캔버스 → CapCut → 9:16 MP4** 쇼츠 파이프라인을 다룹니다. 한국어/영어 거울 구조:

| 부 | 내용 |
|---|---|
| 🇰🇷 [한국어](#-한국어) | ① 큰 그림 → ② 샌드박스 경계(보안) → ③ 사전 준비 (계정·앱·brew) → ④ 1회 세팅 → ⑤ 일상 사용 → ⑥ UI 변경 대응 → ⑦ 트러블슈팅 → ⑧ 약관·저작권 |
| 🇬🇧 [English](#-english) | Same content, mirror |

> 🎯 **첫 방문 권장**: 위 5분 빠른 시작 (TL;DR) 만 따라하면 끝. 좌표 배치가 깨지면 `SHORTS_HEADED=1` 헤드모드로 1분만 손으로 — 단순 슬라이드쇼는 ffmpeg 폴백으로 **완전 무인** 가능. **OpenClaw 본체와 무관**하게 호스트 `~/openclaw-shorts/` 안에서만 동작합니다.
