...
- `docker compose exec openclaw-cli bash` → `claude` — 컨테이너 안에서 직접 CLI (네트워크 무관하게 STDIN/STDOUT)
- 워크스페이스 파일 읽기·쓰기
...
웹UI 가 필요 없고 터미널만 쓰면 `isolated` 그대로 가능 — `./openclaw chat` 또는 `docker compose exec openclaw-cli bash → claude` 는 네트워크 publish 와 무관하게 동작합니다.
</details>
