...
| 11 | **Skills / 플러그인 / 채널** | 추가 능력(이미지 생성, 음성) + 채널 연동 (Discord, Telegram, WhatsApp 등) | 기본값 또는 `skip` — 처음엔 가볍게. Discord 봇 연동은 별도 가이드 → [💬 GUIDE-DISCORD-BOT](docs/GUIDE-DISCORD-BOT.md) |
...
<details>
<summary><b>웹UI 페이지는 떴는데 본문이 까만 빈 화면이에요 — 채팅창이 안 보여요</b></summary>

`http://127.0.0.1:18789` 가 열렸고 탭 제목이 `OpenClaw Control` 인데 본문이 검은 빈 화면이라면 — **버그 아니라 정상**입니다. 이 빌드의 웹 UI 는 **관리 콘솔(Control Panel)** 이고 채팅 인터페이스는 들어있지 않을 수 있어요. GUIDE-FIRST-USE.md 도 "OpenClaw 본체 버전에 따라 UI 가 내장되어 있을 수도, API 만 있을 수도 있음" 이라고 명시.
...
<details>
<summary><b>웹UI(<code>http://127.0.0.1:18789</code>) 가 "Safari Can't Connect" 로 뜨는데 컨테이너는 떠 있어요</b></summary>
