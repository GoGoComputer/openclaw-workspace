...
| 💬 **Run OpenClaw as a Discord bot** | [docs/GUIDE-DISCORD-BOT.md](docs/GUIDE-DISCORD-BOT.md) | **Setup** (create app → Message Content Intent → OAuth2 invite → paste token → first test) **+ everyday use** (four ways to trigger: mention/DM/slash/whitelist · prompt patterns · per-channel model & persona · workspace file references · mute/disable/multi-server · cheat sheet) **+ security + 8-case troubleshooting**. |
| 🔄 **Daily on/off cycle (after install)** | [docs/GUIDE-DAILY-USE.md](docs/GUIDE-DAILY-USE.md) | The everyday loop once setup is done — morning start, stepping away, shutdown, resuming a session, what to do when something looks wrong, periodic maintenance. Side-by-side table comparing `./openclaw stop` vs `docker compose down` vs `uninstall`. |
